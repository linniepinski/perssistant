...
class Fre_Notification extends AE_PostAction
{
    ...
    function bidDeleted($bid_id) {
        ...
        $notification = array(
            ...
            'post_status' => 'publish',
            'post_author' => $bid->post_author,
            'post_title' => sprintf(__("Bid on project %s was declined", 'notification-backend') , get_the_title($project->ID))
        );
        return $this->insert($notification);
    }
    ...
    function fre_notify_item($notify) {      
        ...
        switch ($type) {
            ...
            case 'bid_deleted':
                ...
                $content.= sprintf(__("Your bid at %s was declined by %s", 'notification-backend') , $project_link, $author);
                break;
