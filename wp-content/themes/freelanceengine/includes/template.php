...
function ae_edit_post_button ($post) {
    ...
    <?php if($post->post_status == 'archive'){ ?>
        ...
        <a title="<?php _e("Delete", 'template-backend'); ?>" data-action="delete" class="action archive" href="#">
            <i class="fa fa-times"></i>
        </a>
    <?php } ?>
    <?php if($post->post_status == 'archive'){ ?>
        <a title="<?php _e("Renew", 'template-backend'); ?>" data-target="#" class="" href="<?php echo et_get_page_link('submit-project', array('id' => $post->ID)) ?>">
