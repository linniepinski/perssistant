...
<script type="text/template" id="ae-bid-history-loop">
    <div class="name-history">
        ...
        <div class="content-bid-item-history">
            ...
            <span class="stt-in-process"><?php _e('Job in process', 'modal-work-history-item');?></span>
            <# }else if(post_status == 'close') { #>
            </h5>
            <span class="stt-in-process"><?php _e('Job is closed', 'modal-work-history-item');?></span>
            <# } #>
        </div>
    </div>
    <ul class="info-history">
        ...
        <# if(bid_budget) { #>
            <li><?php _e('Bid Budget :', 'modal-work-history-item') ; ?> <span class="number-price-project-info"> {{= bid_budget_text }} </span></li>
        <# } #>
    </ul>
...
//if(fre_share_role() || ae_user_role($author_id) != FREELANCER ) { ?>
    ...
    <ul class="info-history">
        ...
        <# if(budget) { #>
            <li><?php _e('Budget :', 'modal-work-history-item') ; ?> <span class="number-price-project-info"> {{= budget }} </span></li>
        <# } #>
        <li class="post-control"> 
            {{= status_text }} <span> &#9830; </span>
            <?php ae_js_edit_post_button(); ?>
        <# if(typeof workspace_link !== 'undefined') { #>
            <a href = "{{= workspace_link }}" title=" <?php _e( 'Open Workspace' , 'modal-work-history-item' ); ?>">
