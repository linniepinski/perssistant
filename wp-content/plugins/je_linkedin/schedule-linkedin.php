...
<div id="linkedin-schedule" class="et-main-main clearfix" <?php if ( !isset($sub_section) || $sub_section != 'schedule') echo 'style="display:none"' ?>  >
	<div class="module">
		...
		<div class="desc">
			...
			<div class="form no-margin no-padding no-background">
				<div class="form-item">
					<input style="width : 50%;" class="bg-grey-input " type="text" value="<?php echo get_option('et_linkedin_recurrence',24) ?>" id="recurrence" name="recurrence"> (hours)
				</div>
			</div>
	...
	<div class="module">
		...
		<div class="desc">
			<div class="module form-schedule" id="form-schedule" style="display: none">
				<form action="#" method="post" name="import_linkedin_schedule_form" id="linked_schedule_form">
					<div class="form-item">
						...
						<div class="form no-margin no-padding no-background">
								<div class="select-category select-style et-button-select">
									<select name="category" class="" title="<?php _e("ALL CATEGORIES", ET_DOMAIN); ?>">
										<option class="empty" value=""><?php _e("ALL CATEGORIES", ET_DOMAIN); ?> </option>
										<?php foreach ($industry as $key => $value) { ?>
											<option value="industry,<?php echo $key ?>" >
												<?php echo $value ?>
											</option>
										<?php } ?>
									</select>
						...
						<div class="select-category et-button-select select-style">
							<select name="countrycode" class="" title="<?php _e("Choose your country", ET_DOMAIN); ?>">
					...
					<div class="form-item">
						<input type="text" name="jobtitle" placeholder="<?php _e('Job title', ET_DOMAIN) ?>" value="" id="linkedin_jobtitle_input" />
					</div>
					
					<div class="form-item" >
						<input type="text" name="keywords" placeholder="<?php _e('Keywords', ET_DOMAIN) ?>" value="" id="linkedin_keywords_input" />
					</div>
					<div class="form-item" >
						<input type="text" name="postalcode" placeholder="<?php _e('Postal code', ET_DOMAIN) ?>" value="" id="linkedin_postalcode_input" />
					</div>
					<div class="form-item">
						<input type="text" name="companyname" placeholder="<?php _e('Company name', ET_DOMAIN) ?>" value="" id="linkedin_companyname_input" />
					</div>
					<div class="form-item">
			...
			<div class="btn-add-importer">
				<button id="add-linkedin-schedule"><?php _e("Add", ET_DOMAIN); ?> <span class="icon" data-icon="+"></span></button>
			</div>
		</div>
		<?php 
		...
		<table class="list-import-job" id="schedule_list">
			<tr>
				...
				<th width=""><?php _e("Job Title", ET_DOMAIN); ?></th>
				 <th width=""><?php _e("Category", ET_DOMAIN); ?></th>
				<th width=""><?php _e("Keywords", ET_DOMAIN); ?></th>
				<th width=""><?php _e("Country", ET_DOMAIN); ?></th>
				<th width=""><?php _e("Postal Code", ET_DOMAIN); ?></th>
				
				<th width=""><?php _e("Company Name", ET_DOMAIN); ?></th>
			...
			foreach ($schedules as $i => $row) {
				...
				?>
					<tr class="<?php if(!$sche['active']) echo 'off' ?>" id="schedule-<?php echo $sche['id'] ?>">
						...
						<td><?php echo $sche['category']; ?></td>
						<td><?php echo $sche['keywords'] ?></td>
						<td><?php echo $country ;?></td>
						<td><?php echo $sche['postalcode'] ?></td>
						
						<td><?php echo $sche['companyname'] ?></td>
						<td>
