<?php
/*
Plugin Name: CE Coupon
Plugin URI: www.enginethemes.com
Description: CE Coupon available on CE version 1.5.4
Version: 1.3
...
define('CE_COUPON_VERS', '1.2');

require_once dirname(__FILE__) . '/update.php';

add_action ('after_setup_theme', 'ce_coupon_menu');
function ce_coupon_menu () {
	require_once dirname(__FILE__) . '/admin_view.php';
	if(class_exists('CE_Coupon_Menu'))
		new CE_Coupon_Menu();
...
class CE_Coupon {
	function __construct() {
		...
		add_action('before_ce_payment_button', array($this,'add_coupon_form'));

		//add_action('wp_footer', array($this, 'front_end_script'));
		//add_action('wp_head', array($this, 'front_end_styles'));


		add_action('ce_on_add_scripts', array($this, 'front_end_script'));
		add_action('ce_on_add_styles', array($this, 'front_end_styles'));




		add_filter( 'et_payment_setup_order_data', array($this,'setup_order_data') );
		add_filter( 'et_payment_setup', array($this,'ce_payment_setup'), 50, 3 ) ;
	...
	*/
	function ce_payment_setup ($response, $paymentType, $order) {
		...
		if($order_pay['total'] <= 0 ) {
			...
			$response	=	array(
				'success' 		=> true,
				'data'	=> array ('ACK' => true, 'url' => et_get_page_link('process-payment' , array('paymentType' => 'coupon' , 'ad_id' => $ad_id)) )
			);
	...
	*/
	function add_coupon_form () {
		...
		?>
			<li class="clearfix  payment-coupon payment-button" data-type="coupon">
				<div id="coupon_form" class="">
					<div class="coupon-item">
						<div class="coupon-input">
							<input type="text" class="bg-default-input" placeholder="<?php _e('ENTER YOUR COUPON',ET_DOMAIN);?>" id="coupon_code" />
							<span class="total-title"><?php _e('Total price', ET_DOMAIN );?></span>
							<span class="ad-price"></span>
							<span class="coupon-price"></span>
							<button type="button" class="btn btn-primary btn-post-free disable-payment"><?php _e('Free Post',ET_DOMAIN);?></button>
						</div>
					</div>
				</div>
			</li>
	...
	*/
	function check_coupon ( $code , $user_ID = 0 ) {
		...
		*/
		$used_coupon	=	intval(get_user_meta( $user_ID, 'ce_coupon_used_'.$code , true ));
	...
	*/
	function update_coupon ($extends = array()) {
		...
		*/
		if(isset($extends['coupon_id']) && $extends['coupon_id'] ) {
			...
			update_post_meta( $id, 'ce_coupon_code', $coupon_code );
		}
		unset($extends['coupon_id']);
		foreach ($extends as $key => $value) {
			update_post_meta( $id, 'ce_coupon_'.$key, $value);
		}

		if($id && !is_wp_error( $id )) {
			return array('id' => $id, 'code' => get_post_meta( $id, 'ce_coupon_code', true ));
		} else {
			return false;
		}
	}
	...
	function register_post_type () {
		register_post_type('ce_coupon', array(
			'labels' => array(
	...
	function front_end_script() {
		...
		if(is_page_template('page-post-ad.php') || is_page_template( 'page-upgrade-account.php' )) {
			...
			wp_enqueue_script('coupon_frontend', plugin_dir_url( __FILE__).'/js/front-end.js',array('jquery', 'jquery-ui-autocomplete', 'underscore','backbone','ce') ,CE_VERSION, true);
			wp_localize_script( 'coupon_frontend','ce_coupon', array('currency'	 => ET_Payment::get_currency()) );
		}
	}
	...
	function front_end_styles () {
		if(is_page_template('page-post-ad.php') || is_page_template( 'page-upgrade-account.php' )) 
			wp_enqueue_style( 'ce_coupon.frontend.style', plugin_dir_url(__FILE__).'/css/front-end.css' );
	}

	function plugin_styles () {
		wp_enqueue_style('admin_styles');
		wp_enqueue_style( 'ce_coupon', plugin_dir_url(__FILE__).'/css/ce-coupon.css', array('admin_styles') );
	}

}

class CE_Coupon_Ajax extends CE_Coupon {
	function __construct () {
		parent::__construct();
		...
		if(isset($_POST['coupon_code']))  {
			...
			if(!isset($is_available['success'])) {
				...
				if(empty($added_product)) {
					...
					wp_send_json( array('success' => true , 'data' => $is_available['coupon_code'] , 'coupon_price' => max($coupon_price,0) , 'msg' => __('Your coupon available 1', ET_DOMAIN) ));
					exit;
				}
				...
				foreach ($added_product as $key => $value) {
					...
					if( $key == $_POST['packageID'] ) {
						...
						wp_send_json( array('success' => true , 'data' => $is_available['coupon_code'] , 'coupon_price' => max($coupon_price,0), 'msg' => __('Valid coupon', ET_DOMAIN) ) );
						exit;
					}

				}

				$result = (array('success' => false , 'data' => $is_available['coupon_code'] , 'msg' => __("Your coupon code is not applicable for the selected payment plan.", ET_DOMAIN) ));
				wp_send_json($result);
			}
			else {
				wp_send_json($is_available);
			}
		}
	}
...
class CE_Coupon_Schedule extends CE_Coupon {
