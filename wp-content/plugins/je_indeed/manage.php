<div id="indeed-manage" class="et-main-main inner-content import-container clearfix" <?php if ( !isset($sub_section) || $sub_section != 'manage') echo 'style="display:none"'?>>
	<div class="title font-quicksand"><?php _e('Plugin options', ET_DOMAIN) ?></div>
	<div class="desc">
		<form id="indeed_options">
			<div class="form no-margin no-padding no-background">
				<div class="form-item">
					<input class="option" type="checkbox" name="display_label" <?php if (get_option('et_indeed_display_label') == 1) echo 'checked="checked"' ?> value="1" class="checkbox check-display">
					<?php _e('Display label "Job by indeed"') ?>
				</div>
			</div>
			<div>
				<?php _e('Automatically delete imported job older than ') ?>
				<input type="text" class="option delete-limit-days" style="width: 50px" name="delete_days" value="<?php echo get_option('et_indeed_delete_days', '') ?>" maxlength="3">
	...
	<div class="desc">
		...
		<div class="indeed-controls">
			...
			<?php } ?>
			<a href="#" id="delete_indeed" class="icon" data-icon="#"> <?php _e('Delete selected jobs', ET_DOMAIN) ?></a>
		</div>
	</div>
</div>